...
<div id="titulo-admin">
    ...
    <div id="migajas-titulo"><i class="fa fa-tags"> </i> PRODUCTOS &nbsp;<i class="fa fa-angle-right"> </i>&nbsp; VER</div>
...
<div id="elemento-admin">
            ...
            <div id="texto-titulo" style="margin: 15px 0px 15px 0px;"> <i class="fa fa-tags"> </i> INFORMACIÓN DEL PRODUCTO</div>

            <div id="contenedor-producto">
                <div class="titulo-venta"><b>Producto</b></div>
                        ...
                        <table id="tabla" class="display compact cell-border stripe" style="width:100%">
                            <thead>
                                <tr>
                                    ...
                                    <th class="td-imagen"></th>
                                    <th>Ubicación</th>
                                    <th style="width: 100px; text-align: center;">En Inventario</th>
                                </tr>
                            </thead>
                            <tbody>
                                @php
                                    $contador = 1;
                                @endphp
                                @foreach($producto_sucursales as $producto_sucursal)
                                    <tr onclick="accion('{{$producto_sucursal->id_sucursal}}',`{{ route('sucursales.productos') }}`);" style="cursor: pointer;" title="VER PRODUCTOS EN {{$producto_sucursal->nombre_sucursal}}">
                                        <td style="width: 20px;text-align: center;">{{$contador}}</td>
                                        <td id="td-imagen"><div class="content-imagen-td"><img src="{{ asset('public/imagenes/sistema/sucursales') }}/{{$producto_sucursal->nombre_imagen_sucursal}}" style="height: 100%;"> </div></td>
                                        <td>{{$producto_sucursal->nombre_sucursal}}</td>
                                        <td style="width: 20px; text-align: center; font-weight: bold;">{{$producto_sucursal->cantidad}}</td>
                                    </tr> 
                                    @php $contador = $contador + 1; @endphp
                                @endforeach

                                @foreach($producto_bodegas as $producto_bodega)
                                    <tr onclick="accion('{{$producto_bodega->id_bodega}}',`{{ route('bodegas.productos') }}`);" style="cursor: pointer;" title="VER PRODUCTOS EN {{$producto_bodega->nombre_bodega}}">
                                        <td style="width: 20px;text-align: center;">{{$contador}}</td>
                                        <td id="td-imagen"><div class="content-imagen-td"><img src="{{ asset('public/imagenes/sistema/sucursales/default.png') }}" style="height: 100%;"> </div></td>
                                        <td>{{$producto_bodega->nombre_bodega}}</td>
                                        <td style="width: 20px; text-align: center; font-weight: bold;">{{$producto_bodega->cantidad}}</td>
                                    </tr>
                                    @php $contador = $contador + 1;@endphp
                                @endforeach
