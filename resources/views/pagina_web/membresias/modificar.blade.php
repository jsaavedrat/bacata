...
<div id="titulo-admin">
    ...
    <div id="migajas-titulo"><i class="icon-price-ribbon"> </i> MEMBRESIAS &nbsp;<i class="fa fa-angle-right"> </i>&nbsp; EDITAR</div>
...
<div id="elemento-admin">
    ...
    <div id="texto-titulo"> <i class="icon-pencil"> </i> Editar Membresía en página Web</div>

    <form method="post" data-parsley-validate action="{{ route('pagina.membresias.modificar') }}" enctype="multipart/form-data">
        @csrf
        <div style="width: 100%;float: left;">
            <div class="label-campo">
                ...
                <input type="text" name="modelo_membresia" id="modelo_membresia" class="campo-admin" placeholder="Nombre membresía" spellcheck="false" autocomplete="off" maxlength="50" required value="{{$membresia->nombre_modelo}}">
            </div>
            ...
            <div class="label-campo">
                ...
                <input type="number" class="campo-admin" name="precio_membresia" id="precio_membresia" placeholder="Precio membresía" required value="{{$membresia->precio_base}}">
            </div>
            ...
            <input type="hidden" name="id_producto_anterior" value="{{$membresia->id_producto}}">
            <input type="hidden" name="id_modelo_anterior" value="{{$membresia->id_modelo}}">
            <input type="hidden" name="ids_clasificaciones" value={{$producto_membresias->ids_clasificaciones}}>

        </div>

        <button class="boton-admin" id="crear-servicio"> Editar Membresía</button>
    </form>
</div>
...
<script type="text/javascript">
    ...
    var especificaciones_membresias = {!! json_encode($membresia->especificaciones) !!};
    for(var i=0; i<especificaciones_membresias.length; i++){
        if ($("#clasificacion-"+especificaciones_membresias[i].id_clasificacion).length){
            document.getElementById('clasificacion-'+especificaciones_membresias[i].id_clasificacion).value = especificaciones_membresias[i].nombre_especificacion;
        }
    }

    console.log(especificaciones_membresias);
