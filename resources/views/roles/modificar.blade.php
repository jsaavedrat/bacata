...
<div id="titulo-admin">
    ...
    <div id="migajas-titulo"><i class="icon-creative-commons-attribution"> </i> ROLES &nbsp;<i class="fa fa-angle-right"> </i>&nbsp; EDITAR</div>
...
<div id="elemento-admin">
    <div id="content-mensaje">
        @if($estatus=="exito")<div id="mensaje-exito"> <i class="fa fa-check-circle"></i> ROL Editado Exitosamente</div>@endif
        @if($estatus=="error")<div id="mensaje-error"> <i class="fa fa-times-circle"></i>&nbsp; Nombre del ROL ya existe</div>@endif
    </div>
    <div id="texto-titulo"> <i class="icon-edit"> </i> Editar ROL EMPLEADO</div>

    <form method="post" action="{{ route('roles.modificar') }}">
        @csrf
        ...
        <div class="label-campo">
            ...
            <input type="text" name="nombre_rol" class="campo-admin" id="nombre_rol" placeholder="Nombre rol empleado" spellcheck="false" autocomplete="off" maxlength="30" onkeyup="this.value=letras(this.value)" value="{{$rol->name}}">
        </div>
        ...
        @foreach($modulos as $modulo)
            <div class="content-permiso">
                <div class="modulo-permiso" id="modulo-permiso-{{$modulo->nombre_modulo}}">
                    ...
                    @foreach($permisos_modulos as $permisos_modulo)
                        @if($modulo->id_modulo == $permisos_modulo->id_modulo)
                            <div class="permiso-modulo"><i class="icon-dot-single"></i>
                                ...
                                <input type="checkbox" value="{{$permisos_modulo->id_permiso}}" class="check-{{$modulo->nombre_modulo}} checkPermisos" id="check-permiso-{{$permisos_modulo->id_permiso}}" onclick="verificarTodos('check-{{$modulo->nombre_modulo}}');">
                            </div>
                        @endif
                    @endforeach
                </div>
        ...
        <input type="hidden" id="id_rol" name="id_rol" value="{{$rol->id}}">
        ...
        <button class="boton-admin" id="crear-rol"> Editar Rol de Empleados</button>
    </form>
</div>
...
<script type="text/javascript">
    ...
    permisos_actuales = {!! json_encode($permisos_rol) !!};
    for(var i=0; i<permisos_actuales.length; i++){
        document.getElementById("check-permiso-"+permisos_actuales[i].permission_id).checked = true;
    }

    modulos_permisos = {!! json_encode($modulos_permisos) !!};
    for(var i=0; i<modulos_permisos.length; i++){
        
        var check = "check-"+modulos_permisos[i].nombre_modulo;

        var checks = document.getElementsByClassName(check);
        var c = 0;
        for (j = 0; j < checks.length; j++) {

            var values = document.getElementsByClassName(check)[j].checked;
            if(values==false){
                c = c + 1;
            }
        }
    ...
    var modulos_permisos = document.getElementsByClassName("modulo-permiso");

    for(var i=0; i<modulos_permisos.length; i++){
        
        modulos_permisos[i].click();
