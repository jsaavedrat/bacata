...
jQuery.fn = jQuery.prototype = {
	...
	// Ejecute una devolución de llamada para cada elemento en el conjunto coincidente.
	...
	// Se comporta como un método de matriz, no como un método jQuery.
	empuja empuja,
	tipo: arr.sort,
	empalme: arr.splice
};
...
jQuery.extend = jQuery.fn.extend = function () {
	opciones de var, nombre, src, copy, copyIsArray, clone,
	...
	// Manejar el caso cuando el destino es una cadena o algo (posible en copia profunda)
	...
	para (; i <longitud; i ++) {
		...
		// Solo trata con valores no nulos / indefinidos
		if ((opciones = argumentos [i])! = nulo) {
			...
			para (nombre en opciones) {
				...
				if (profundo && copy && (jQuery.isPlainObject (copy) ||
...
jQuery.extend ({
	...
	isPlainObject: function (obj) {
		...
		// Use toString en lugar de jQuery.type para atrapar objetos host
		...
		// Los objetos con prototipo son simples si fueron construidos por una función Objeto global
	...
	// push.apply (_, arraylike) arroja sobre WebKit antiguo
	...
	map: function (elems, callback, arg) {
		...
		// Revisa todas las teclas del objeto,
		} más {
			para (i en elems) {
				valor = devolución de llamada (elems [i], i, arg);
		...
		// Acoplar cualquier matriz anidada
	...
	// propiedades a él, por lo que debe existir.
	soporte soporte
});
...
La función isArrayLike (obj) {

	// Soporte: solo iOS 8.2 real (no reproducible en simulador)
	// la verificación `in` se usa para evitar el error JIT (gh-2145)
	...
	tipo de retorno === "matriz" || longitud === 0 ||
		typeof length === "número" && length> 0 && (length - 1) en obj;
}
chisporroteo var =
/ *!
 ...
 * https://sizzlejs.com/
 * *
 * Copyright jQuery Foundation y otros contribuyentes
 * Publicado bajo la licencia MIT
...
var i,
	...
	push = arr.push,
	slice = arr.slice,
	// Use un indexOf despojado ya que es más rápido que el nativo
	...
	booleans = "verificado | seleccionado | asíncrono | autofocus | reproducción automática | controles | diferir | deshabilitado | oculto | ismap | loop | múltiple | abierto | readonly | requerido | alcance",
	...
	pseudos = ":(" + identificador + ") (?: \\ ((" +
		// Para reducir el número de selectores que necesitan tokenizar en el preFiltro, prefiera argumentos:
	...
	// Espacio en blanco al inicio y no escapado, capturando algunos caracteres que no son espacios en blanco que preceden a este último
	rwhitespace = new RegExp (espacio en blanco + "+", "g"),
	rtrim = nuevo RegExp ("^" + espacio en blanco + "+ | ((?: ^ | [^ \\\\]) (?: \\\\.) *)" + espacio en blanco + "+ $", "g "),

	rcomma = new RegExp ("^" + espacio en blanco + "*," + espacio en blanco + "*"),
	rcombinators = new RegExp ("^" + espacios en blanco + "* ([> + ~] |" + espacios en blanco + ")" + espacios en blanco + "*"),

	rattributeQuotes = new RegExp ("=" + espacio en blanco + "* ([^ \\] '\"] *?) "+ espacio en blanco +" * \\] "," g "),

	rpseudo = nuevo RegExp (pseudos),
	...
	matchExpr = {
		...
		"CHILD": new RegExp ("^ :( only | first | last | nth | nth-last) - (child | of-type) (?: \\ (" + whitespace +
			"* (par | impar | (([+ -] |) (\\ d *) n |)" + espacios en blanco + "* (?: ([+ -] |)" + espacios en blanco +
			"* (\\ d +) |))" + espacio en blanco + "* \\) |)", "i"),
		"bool": nuevo RegExp ("^ (?:" + booleans + ") $", "i"),
		// Para uso en bibliotecas que implementan .is ()
		// Utilizamos esto para la coincidencia de POS en `select`
		"needsContext": nuevo RegExp ("^" + espacio en blanco + "* [> + ~] |: (par | impar | eq | gt | lt | nth | primero | último) (?: \\ (" +
	...
	// ID fácilmente identificables / recuperables o selectores TAG o CLASS
	...
	funescape = function (_, escaped, escapedWhitespace) {
		...
		volver alto! == alto || escapadoWhitespace?
			escapado :
			alto <0?
				// punto de código BMP
	...
	fcssescape = function (ch, asCodePoint) {
		if (asCodePoint) {
			...
			// U + 0000 NULL se convierte en CARACTER DE REEMPLAZO U + FFFD
			if (ch === "\ 0") {
				devuelve "\ uFFFD";
			}
		...
		// Otros caracteres ASCII potencialmente especiales consiguen una barra invertida
		devuelve "\\" + ch;
	},
	...
	disabledAncestor = addCombinator (
		función (elem) {
			return elem.disabled === verdadero && ("formulario" en elem || "etiqueta" en elem);
		},
...
tratar {
	...
	// Soporte: Android <4.0
	// Detecta push.apply silenciosamente fallando
	arr [preferredDoc.childNodes.length] .nodeType;
} captura (e) {
	push = {apply: arr.length?

		// Aprovecha el segmento si es posible
...
función Sizzle (selector, contexto, resultados, semilla) {
	var m, i, elem, nid, match, groups, newSelector,
		...
		// nodeType tiene un valor predeterminado de 9, ya que el contexto predeterminado es el documento
		nodeType = context? context.nodeType: 9;

	resultados = resultados || [];

	// Regrese temprano de las llamadas con un selector o contexto no válido
	...
	si (! semilla) {
		...
		if ((context? context.ownerDocument || context: preferredDoc)! == documento) {
